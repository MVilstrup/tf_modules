...
class VGG(BaseModel):
    ...
    def maybe_download_weights(self):
        ...
        if not self.config.checkpoint_file:
            checkpoint_file = '{}/{}.ckpt'.format(model_path, self.name)
        ...
        if not os.path.exists(checkpoint_file):
            ...
            os.rename('{}.ckpt'.format(self.name), checkpoint_file)
            os.remove(file_name)
    ...
    def vgg19(self, inputs, spatial_squeeze=True, scope='vgg_19', fc_conv_padding='VALID', reuse=False):
        self.name = 'vgg_19'
        self.url = 'http://download.tensorflow.org/models/vgg_19_2016_08_28.tar.gz'
        if self.config.use_initial_weights:
            self.maybe_download_weights()

        with slim.arg_scope(self.arg_scope()):
            return self._vgg_19(inputs, spatial_squeeze=True, scope='vgg_19', fc_conv_padding='VALID', reuse=reuse)

    def _vgg_19(self, inputs, spatial_squeeze=True, scope='vgg_19', fc_conv_padding='VALID', reuse=False):
        with tf.variable_scope(scope, 'vgg_19', [inputs]) as sc:
            ...
            with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
                ...
                net = slim.dropout(net, self.config.dropout_keep_prob, is_training=self.config.is_training,
                                   scope='dropout6')
...
class VGG16(VGG):
    ...
    def build(self):
        with tf.variable_scope(self.scope, 'vgg_16', [self.inputs]) as sc:
            end_points_collection = sc.name + '_end_points'
            # Collect outputs for conv2d, fully_connected and max_pool2d.
            with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
                                outputs_collections=end_points_collection):
