...
from tf_modules.assertions.check import *
...
class VGG(BaseModel):
    ...
    def maybe_download_weights(self):
        ...
        if not os.path.exists(checkpoint_file):
            ...
            model_name = '{}.ckpt'.format(file_name.split('.')[0])
            os.system("curl -O '{}'".format(self.url))
            os.system("tar -xf '{}'".format(file_name))
            os.rename('{}.ckpt'.format(model_name), checkpoint_file)
            os.remove(file_name)
...
class VGG16(VGG):
    def __init__(self, config, inputs, should_be_extended=True, reuse=None, spatial_squeeze=True,
        ...
        with slim.arg_scope(self.arg_scope(), reuse=reuse):
            self.build()
...
class VGG19(VGG):
    ...
    def build(self):
        with tf.variable_scope(self.scope, 'vgg_19', [self.inputs]) as sc:
            end_points_collection = sc.name + '_end_points'
            # Collect outputs for conv2d, fully_connected and max_pool2d.
            with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
                                outputs_collections=end_points_collection):
